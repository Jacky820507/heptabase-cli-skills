---
name: heptabase-cli
description: Interact with Heptabase using the CLI to create, read, and edit notes, journals, tags, cards, tag property schemas, card property values, list whiteboards and manage cards on whiteboards, and browse AI Tutor goals, courses, and lessons. Use when the user asks to manage their Heptabase knowledge base, search cards, work with journals, tags, tag database properties, card property values, or whiteboards, or read AI Tutor content.
...
- **Mutations are serialized.** Write operations (create, save, append, trash, restore, tag add/remove, card set-property, whiteboard add-card/remove-card) run one at a time to prevent conflicts. Reads are concurrent.
